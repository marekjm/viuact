			  ...
			  void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_PROTOCOL_INIT:
		vhd = static_cast<vhd_minimal_server_echo*>(lws_protocol_vh_priv_zalloc(lws_get_vhost(wsi),
				lws_get_protocol(wsi),
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		...
		do {
			...
			pmsg = static_cast<msg const*>(lws_ring_get_element(pss->ring, &pss->tail));
			if (!pmsg) {
...

            {
                ...
                auto const length = pmsg->len;

                received_message.resize(length + 1);
                for (auto i = size_t{0}; i < length; ++i) {
                    received_message.at(i) = *(static_cast<char const*>(pmsg->payload) + LWS_PRE + i);
                }

                auto message = std::string{received_message.data()};
                std::lock_guard<std::mutex> lck{from_client_mtx};
                from_client.push(std::move(message));
        ...
        {
            std::unique_lock<std::mutex> lck{to_client_mtx};
            if (not to_client.empty()) {
                auto const message = std::move(to_client.front());
                to_client.pop();
                lck.unlock();

                auto buffer = std::vector<uint8_t>{};
                buffer.resize(LWS_SEND_BUFFER_PRE_PADDING + message.size() + LWS_SEND_BUFFER_POST_PADDING);

                for (auto i = size_t{0}; i < message.size(); ++i) {
                    buffer.at(LWS_SEND_BUFFER_PRE_PADDING + i) = message.at(i);
                }
